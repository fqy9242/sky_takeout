...
import com.fresh.exception.OrderBusinessException;
...
import com.fresh.utils.WeChatPayUtil;
import com.fresh.vo.*;
...
/*		//支付状态
	...
	public void checkOutOfRange(String address) {
		...
		//获取店铺的经纬度坐标
		String shopCoordinate = HttpUtil.get("https://api.map.baidu.com/geocoding/v3", map);

		JSONObject jsonObject = JSONUtil.parseObj(shopCoordinate);
		...
		//获取用户收货地址的经纬度坐标
		String userCoordinate = HttpUtil.get("https://api.map.baidu.com/geocoding/v3", map);

		jsonObject = JSONUtil.parseObj(userCoordinate);
		if(!jsonObject.getStr("status").equals("0")){
		...
		lat = location.getStr("lat");
		lng = location.getStr("lng");
		//用户收货地址经纬度坐标
		...
		//路线规划
		String json = HttpUtil.get("https://api.map.baidu.com/directionlite/v1/driving", map);
		...
		cn.hutool.json.JSONArray jsonArray = result.getJSONArray("routes");
		Integer distance = jsonArray.getJSONObject(0).getInt("distance");
